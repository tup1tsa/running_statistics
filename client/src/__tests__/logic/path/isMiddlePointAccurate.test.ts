...
import { getDistance } from "../../../containers/geoLibHelpers";
...
it("should return true if third point further away from 1-st than second", () => {
  ...
  expect(isMiddlePointAccurate(start, middle, end, getDistance)).toBe(true);
});
...
it("should return false if third point near the first point. Second point is very far away", () => {
  ...
  expect(isMiddlePointAccurate(start, middle, end, getDistance)).toBe(false);
});
