...
import { SparsePolyline } from "../../../application/components/GoogleMap/SparsePolyline";
...
describe("sparse polyline logic", () => {
  ...
  const firstPolyline = (
    <Polyline
      path={firstPolylinePath}
      options={{ strokeColor: path[0].color }}
    />
  );
  const secondPolylinePath = [{ lat: 22, lng: 17 }];
  const secondPolyline = (
    <Polyline
      path={secondPolylinePath}
      options={{ strokeColor: path[1].color }}
    />
  );
