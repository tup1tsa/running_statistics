...
describe('download races logic', () => {
  ...
  it('should throw an error if server responded with an error', async (done) => {
    ...
    await expect(downloadRaces(axios, jest.fn())).rejects.toEqual(new Error('server is not available'));
    await expect(downloadRaces(axios, jest.fn())).rejects.toEqual(new Error('data is invalid'));
    done();
  });
